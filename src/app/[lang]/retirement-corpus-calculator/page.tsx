  ...
  return (
    <div className="py-12">
      ...
      <div className="mx-auto max-w-5xl">
        ...
        {retirementDict?.related_calculators && <Card className="mt-12 shadow-lg bg-blue-50/50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800">
          <CardHeader>
            <h2 className="text-2xl font-bold">{retirementDict.related_calculators.title}</h2>
            <p className="text-muted-foreground mt-2">{retirementDict.related_calculators.description}</p>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {retirementDict.related_calculators.tools.map((tool: { name: string; description: string; link: string }, index: number) => (
                <Link
                  key={index}
