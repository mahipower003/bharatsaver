...
import MermaidChart from "@/components/calculators/MermaidChart";

// Fallback empty dictionary for the calculator tool if it expects one
const dummyToolDict = {
  title: "LIC Maturity Calculator",
  description: "Calculate your estimated maturity value based on Sum Assured, Term, and Bonus.",
  sum_assured_label: "Sum Assured (₹)",
  policy_term_label: "Policy Term (Years)",
  bonus_rate_label: "Bonus Rate (per ₹1000)",
  fab_rate_label: "Final Additional Bonus Rate (per ₹1000)",
  calculate_button: "Calculate Maturity"
};
    ...
    <div className="space-y-4">
      {content.map((item, idx) => {
        switch (item.type) {
          ...
          case 'mermaid':
             return (
               <MermaidChart key={idx} chart={item.content} />
             );
           case 'faq':
...
export default function CalculateLicMaturityClient({
  params,
  dictionary,
  pageDict,
}: {
  ...
  const pageUrl = `${siteUrl}/${params.lang}/calculate-lic-maturity-amount`;
    ...
    <CalculatorPageLayout
      ...
      calculator={
        <div id="calculator-widget">
          {/* We reuse the existing LicMaturityCalculator here */}
          <LicMaturityCalculator dictionary={pageDict.tool && Object.keys(pageDict.tool).length > 0 ? pageDict.tool : dummyToolDict} />
        </div>
      }
      faqs={faqs}
