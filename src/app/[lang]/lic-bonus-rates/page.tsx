...
import LicBonusRatesClient from "./LicBonusRatesClient";
...
export async function generateMetadata({ params }: { params: Promise<{ lang: Locale }> }): Promise<Metadata> {
    ...
    const pageUrl = `${siteUrl}/${lang}/lic-bonus-rates`;

    let pageDict: any;
    try {
        pageDict = (await import(`@/dictionaries/${lang}/lic-bonus-rates.json`)).default;
    } catch {
        pageDict = (await import(`@/dictionaries/en/lic-bonus-rates.json`)).default;
    }
    ...
    return {
        ...
        description,
        alternates: {
            canonical: `${siteUrl}/en/lic-bonus-rates`,
            languages: i18nConfig.locales.reduce((acc, locale) => {
                acc[locale] = `${siteUrl}/${locale}/lic-bonus-rates`;
                return acc;
...
export default async function LicBonusRatesPage({ params }: { params: Promise<{ lang: Locale }> }) {
    const { lang } = await params;
    ...
    const breadcrumbSchema = {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 3, "name": "LIC Bonus Rates 2026", "item": pageUrl }
        ]
    };
