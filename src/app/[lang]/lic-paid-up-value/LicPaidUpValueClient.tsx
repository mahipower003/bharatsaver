...
import { LicPaidUpValueCalculator } from "@/components/calculators/LicPaidUpValueCalculator";
import { ShieldCheck, Users, HelpCircle, FileText, AlertTriangle, CheckCircle, SlidersHorizontal, GitCompareArrows } from "lucide-react";

function getIcon(iconName: string) {
  const icons: { [key: string]: React.ElementType } = {
    ShieldCheck, Users, HelpCircle, FileText, AlertTriangle, CheckCircle, SlidersHorizontal, GitCompareArrows
  };
    ...
    <div className="space-y-4">
      {content.map((item, idx) => {
        switch (item.type) {
          case 'image':
            return (
              <div key={idx} className="my-8 w-full relative h-[280px] md:h-[380px] rounded-xl overflow-hidden shadow-lg border">
            ...
            return <div key={idx} className="space-y-4 mt-4">
              {item.items.map((step: any, stepIdx: number) => (
                <div key={stepIdx} className="flex gap-4">
                  <div className="flex-shrink-0 w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold">{stepIdx + 1}</div>
                  <div>
          ...
          case 'table':
            return <div key={idx} className="overflow-x-auto rounded-lg border mt-4">
              <Table>
                <TableHeader><TableRow className="bg-muted/50">{item.headers.map((h: string, hIdx: number) => <TableHead key={hIdx} className="font-semibold whitespace-nowrap" dangerouslySetInnerHTML={{ __html: String(h) }}></TableHead>)}</TableRow></TableHeader>
              ...
              {item.footer && <div className="text-sm text-muted-foreground mt-3 px-4 pb-3" dangerouslySetInnerHTML={{ __html: item.footer }} />}
            </div>;
          case 'cta_card':
            return (
            ...
            return (
              <Accordion key={idx} type="single" collapsible className="w-full">
                {item.items.map((faq: { q: string; a: string }, qIdx: number) => (
                  <AccordionItem key={qIdx} value={`faq-${qIdx}`}>
                    <AccordionTrigger className="text-left font-semibold">{faq.q}</AccordionTrigger>
                    <AccordionContent>
                      <div className="prose dark:prose-invert max-w-none text-muted-foreground" dangerouslySetInnerHTML={{ __html: faq.a }} />
                    </AccordionContent>
                  </AccordionItem>
                ))}
              </Accordion>
...
export default function LicPaidUpValueClient({
  params,
  dictionary,
  pageDict,
  breadcrumbSchema,
  ...
  const pageUrl = `${siteUrl}/${params.lang}/lic-paid-up-value`;
